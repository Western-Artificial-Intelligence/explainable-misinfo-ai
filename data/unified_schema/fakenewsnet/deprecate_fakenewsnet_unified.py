...
Usage:
    ...
    --test: Process only first 50 articles for testing
...
import numpy as np
from data.unified_schema.fakenewsnet.deprecate_article_scraping import threaded_hydrate, hydrate_claim
...
def fakenewsnet_schema(
    ...
    # Best-practice: robust URL column detection.
    # Your raw FakeNewsNet CSVs are not always consistent (often "url" vs "news_url").
    # We normalize whichever one exists into a single "news_url" column.
    if "news_url" not in temp.columns:
        if url_col in temp.columns:
            temp.rename(columns={url_col: "news_url"}, inplace=True)
        else:
            for cand in ["url", "news_url", "newsURL", "link", "news_link", "article_url"]:
                if cand in temp.columns:
                    temp.rename(columns={cand: "news_url"}, inplace=True)
                    break

    # If we still don't have URLs, keep the column (all hydrations will become title_only)
    if "news_url" not in temp.columns:
        temp["news_url"] = None
    ...
    # Vectorized content_status based on final text length
    final_len = temp["article_text_final"].fillna("").astype(str).str.len()
    temp["content_status"] = np.select(
        [final_len < 50, final_len < 500],
        ["title_only", "partial"],
    ...
    # - has_text: final output has any meaningful text
    # - is_hydrated: text was fetched by the hydrator (not merely present in CSV)
    temp["has_text"] = final_len >= 50
    if "fetch_status" in temp.columns:
        temp["is_hydrated"] = temp["fetch_status"].isin(["success", "archived_success"])
    else:
        temp["is_hydrated"] = False

    # Extract source domain from news_url (handles missing scheme)
    if "source_domain" not in temp.columns or temp["source_domain"].isna().all():
        def _safe_domain(u):
            ...
            parsed2 = urlparse("http://" + s)
            return (parsed2.netloc or "").lower()

        temp["source_domain"] = temp["news_url"].apply(_safe_domain)

    # --- compute this BEFORE dropping needs_hydration ---
    temp["_had_existing_text"] = ~temp["needs_hydration"]

    temp.drop(
        columns=["needs_hydration", "article_text_original", "article_text"],
        inplace=True,
        errors="ignore"
    )
    ...
    if "fetch_status" in temp.columns:
        temp.loc[temp["_had_existing_text"] & temp["fetch_status"].isna(), "fetch_status"] = "existing_text"
        temp.loc[(~temp["_had_existing_text"]) & temp["fetch_status"].isna(), "fetch_status"] = "worker_exception"

    temp.drop(columns=["_had_existing_text"], inplace=True, errors="ignore")


    if "is_archived" in temp.columns:
        temp["is_archived"] = temp["is_archived"].fillna(False).astype(bool)
    if "is_hydrated" in temp.columns:
        temp["is_hydrated"] = temp["is_hydrated"].fillna(False).astype(bool)
...
def main(test_mode=False, max_articles=None):
    ...
    -----------
    test_mode : bool
        If True, process only 50 articles
    ...
    if test_mode:
        max_articles = 50
        print("TEST MODE: Processing only 50 articles")
    ...
    df_unified = fakenewsnet_schema(
        ...
        dataset="fakenewsnet",
        title_col="title",
        # FakeNewsNet CSVs in this repo use the column name "news_url".
        # The schema function will normalize it into the unified "news_url" field.
        url_col="news_url",
        text_col="text",
        max_workers=12,
        batch_size=None,
    ...
    final.head(50).to_csv(sample_path, index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--test',
        action='store_true',
        help='Test mode: process only 50 articles'
    )
    parser.add_argument(
        '--max-articles',
        type=int,
