...
class FakeNewsNetAdapter:
    ...
    def load(self, *, cache_dir: Optional[str] = None) -> DatasetDict:
        csv_path = f"hf://datasets/rickstello/FakeNewsNet@{self.revision}/FakeNewsNet.csv"
    ...
    def normalize(
        ...
        *,
        ...
        title_col = pick_first_col(cols, ["title", "headline", "news_title", "article_title"])
        ...
        if title_col is None and text_col is None:
            raise RuntimeError(f"[FakeNewsNetAdapter] No usable text columns found. cols={cols}")

        label_col = pick_first_col(cols, ["real", "label", "veracity", "class", "target"])

        if label_col == "real":
            one_is_fake = False

        if label_col is None:
            raise RuntimeError(f"[FakeNewsNetAdapter] No label column found. cols={cols}")
        ...
        def _map_batch(batch, indices):
            titles = [clean_text(x) for x in batch[title_col]] if title_col else [""] * len(indices)
            texts = [clean_text(x) for x in batch[text_col]] if text_col else [""] * len(indices)
            ...
            label_ids = [map_label_to_4way(x, one_is_fake=one_is_fake) for x in raw_labels]

            ids = [f"{self.name}_train_{i}" for i in indices]
            hashes = [claim_hash(c) for c in claims]

            urls = [clean_text(u) for u in batch[url_col]] if url_col else [""] * len(indices)
