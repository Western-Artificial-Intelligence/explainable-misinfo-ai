...
class FakeHealthAdapter:
    ...
    def fetch(self, cache_dir: str = ".cache") -> Path:
        ...
        candidates = [
            ...
            if p.is_dir() and p.name.lower().startswith("fakehealth-")
        ]
        if not candidates:
            raise RuntimeError(
                f"[FakeHealth] Could not find extracted repo folder in {extract_dir}"
            )
        return candidates[0]
    ...
    def _normalize_reviews(
        self, items: List[Dict[str, Any]], subset: str
    ) -> List[Dict[str, Any]]:
        ...
        for idx, r in enumerate(items):
            ...
            title = first_present(
                r, ["title", "Title", "news_title", "headline", "Headline"]
            )
            summary = first_present(
                r,
                ["summary", "Summary", "review_summary", "description", "Description"],
            )
            url = first_present(r, ["url", "URL", "news_url", "link", "Link"])

            rating_raw = first_present(
                r, ["rating", "Rating", "score", "Score", "overall_rating"]
            )
