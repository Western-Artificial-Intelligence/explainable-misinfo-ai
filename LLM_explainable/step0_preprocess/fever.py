...
class FeverAdapter:
    ...
    def _parquet_data_files(self) -> Dict[str, List[str]]:
        ...
        if not parquet_paths:
            raise RuntimeError(f"[FeverAdapter] No parquet files found at {self.repo_id}@{self.revision}")
        ...
        for p in parquet_paths:
            ...
            elif "/validation/" in pl or "/dev/" in pl or "labelled_dev" in pl or "valid" in pl:
    ...
    def load(self, *, cache_dir: Optional[str] = None) -> DatasetDict:
        ...
        def pick_file(filename: str) -> str:
            if filename in files:
                return filename
            matches = [f for f in files if f.endswith("/" + filename) or f.endswith(filename)]
            if not matches:
    ...
    def normalize(
        ...
        *,
        ...
        for split, ds in dd.items():
            ...
            label_col = pick_first_col(cols, ["label", "Label", "gold_label", "verdict"])
            ...
            mapped = [map_fever_label_to_4way(to_label_str(x)) for x in ds[label_col][:sample_n]]
