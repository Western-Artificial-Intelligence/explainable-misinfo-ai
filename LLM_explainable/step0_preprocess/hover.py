...
class HoverAdapter:
    ...
    def _parquet_data_files(self) -> Dict[str, List[str]]:
        ...
        if not parquet_paths:
            raise RuntimeError(
                f"[HoverAdapter] No parquet files found at {self.repo_id}@{self.revision}"
            )
        ...
        *,
        ...
        for split, ds in dd.items():
            ...
            label_col = pick_first_col(
                cols, ["label", "Label", "verdict", "gold_label"]
            )
            expl_col = pick_first_col(
                cols, ["explanation", "Explanation", "justification"]
            )
            ...
            def _map_batch(batch, indices):
                ...
                explanations = (
                    [clean_text(x) for x in batch[expl_col]]
                    if expl_col
                    else [""] * len(indices)
                )
                evidences = (
                    [clean_text(x) for x in batch[evid_col]]
                    if evid_col
                    else [""] * len(indices)
                )
        ...
        if make_validation_if_missing and "train" in out and "validation" not in out:
            ...
            try:
                ...
                print(
                    f"[HoverAdapter] created validation split from train (stratified): val_frac={val_frac}"
                )
                ...
                print(
                    f"[HoverAdapter] created validation split from train (non-stratified fallback): val_frac={val_frac} err={e}"
                )
