...
def map_label_to_4way(raw: object) -> int:
    ...
    if any(
        k in s for k in ["nei", "not enough", "unknown", "unverified", "insufficient"]
    ):
        return LABEL_NEI
    ...
    def normalize(self, dd: DatasetDict) -> DatasetDict:
        ...
        for split, ds in dd.items():
            ...
            if not claim_candidates:
                raise RuntimeError(
                    f"[CoAIDAdapter] No claim candidates found. cols={cols}"
                )
            ...
            fallback_col = (
                "claim" if ("claim" in cols and claim_col != "claim") else None
            )

            if claim_cov[claim_col] == 0.0:
                peek = ds[claim_col][:20]
                raise RuntimeError(
                    f"[CoAIDAdapter] claim_col='{claim_col}' has 0% usable text. peek={peek}"
                )
            ...
            if not label_candidates:
                raise RuntimeError(
                    f"[CoAIDAdapter] No label candidates found. cols={cols}"
                )
            ...
            if coverages[label_col] == 0.0:
                ...
                raise RuntimeError(
                    f"[CoAIDAdapter] label_col='{label_col}' has 0% mappable labels. peek={peek}"
                )
            ...
            def _map_batch(batch, indices):
                ...
                if fallback_col:
                    ...
                    claims = [
                        p if p else f for p, f in zip(primary_vals, fallback_vals)
                    ]
                else:
                    claims = primary_vals
