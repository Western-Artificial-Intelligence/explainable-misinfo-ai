        ...
        *,
        ...
        for split, ds in dd.items():
            ...
            if claim_col is None or label_col is None or claim_id_col is None:
                raise RuntimeError(
                    f"[SciFactAdapter] Unexpected columns in split='{split}': {cols}"
                )
            ...
            print(
                f"[SciFactAdapter] split={split} label_cov={cov:.3f} join_corpus={join_corpus}"
            )
            ...
            def _map_batch(batch, indices):
                ...
                sent_ids_raw = (
                    batch[sent_ids_col] if sent_ids_col else [[]] * len(indices)
                )
                ...
                if join_corpus and corpus_map is not None and doc_id_col:
                    for j, (doc_raw, sent_ids) in enumerate(
                        zip(doc_ids_raw, sent_ids_raw)
                    ):
                        try:
                ...
                return {
                    ...
                    "evidence_sentences": [
                        json.dumps(x, ensure_ascii=False) for x in sent_ids_raw
                    ],
                    "cited_doc_ids": [
                        json.dumps(x, ensure_ascii=False) for x in cited_raw
                    ],
