...
from .schemas import (EvidenceSnippet, FinalResult, GeneratorResult,
                      GuardrailResult, VerifierResult)
...
def run_claim(
    ...
    *,
    ...
    generator_res = GeneratorResult(
        mode=str(gen_out.mode), content=dict(gen_out.content)
    )
    ...
    citations = _normalize_citations(
        generator_res.content.get("citations")
    ) or _normalize_citations(guard.used_eids)
