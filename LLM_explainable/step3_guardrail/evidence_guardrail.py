...
def _distinct_sources(snippets: List[Dict[str, Any]]) -> int:
    return len(
        {(s.get("url") or "").strip() for s in snippets if (s.get("url") or "").strip()}
    )
...
def evidence_guardrail(
    ...
    *,
    ...
    enough = (
        (num_snips >= min_snippets)
        and (num_sources >= min_sources)
        and (coverage >= min_keyword_coverage)
    )
    ...
    if not enough:
        ...
        if coverage < min_keyword_coverage:
            reason_parts.append(
                f"low_keyword_coverage({coverage:.2f}<{min_keyword_coverage:.2f})"
            )
